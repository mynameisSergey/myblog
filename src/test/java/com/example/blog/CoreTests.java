...
public class CoreTests {
    ...
    public void createData() {
        jdbcTemplate.update("insert into posts(title, text, tags) values ('Special post', 'Special post', 'comment')");
        jdbcTemplate.update("insert into posts(title, text, tags) values ('Special post1', 'Special post1', 'special')");
        maxPostId = jdbcTemplate.queryForObject("select coalesce(max(id), 0) from posts", Long.class);
        jdbcTemplate.update("insert into comments(post_id, text) values (?, 'Special comment')", maxPostId);
